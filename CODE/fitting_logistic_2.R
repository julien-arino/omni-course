...
error_incidence <- function(p_vary, 
  ...
  sol = ode(y = c(N=params$N0), times = data$date, func = RHS_logistic, 
...
GA = ga(
  ...
  fitness = function(x)
    -error_incidence(p_vary = c(r = x[1], K = x[2], N0 = x[3]),
                     ...
                     method = "rk4"),
  parallel = 8,
  lower = c(0.1, 1000000, 1000000),
  upper = c(10, 10000000, max(pop$value)),
  optim = TRUE,
  optimArgs = list(method = "CG"),
  suggestions = c(1, params$K, pop$value[1]),
  popSize = 500,
  maxiter = 200
)
...
png("../FIGS/fit_logistic_2.png", width = 1000, height = 600)
if (plot_blackBG) {
