...
\begin{frame}{An example of Bellman and \r{A}str\"om (MBS 1970)}
    ...
    \begin{center}
        ...
        \begin{tikzpicture}[scale=1.5, transform shape]
          ...
          %% Flows between
          \path [line, very thick, bend left] (C1) to node [midway, above] (TextNode) {$k_2C_1$} (C2);
          \path [line, very thick, bend left] (C2) to node [midway, below] (TextNode) {$k_3C_2$} (C1);
          %% Flows in
          \path [line, very thick] (-1.75\horzskip,0\vertskip) to node [midway, above] (TextNode) {$u$} (C1);
          %% Flows out of
          \path [line, very thick] (C1) to node [midway, left] (TextNode) {$k_1C_1$} (-1\horzskip,1\vertskip);
          \path [line, very thick] (C2) to node [midway, right] (TextNode) {$k_4C_2$} (1\horzskip,1\vertskip);
        \end{tikzpicture}    
    \end{center}  
\end{frame}
...
\begin{frame}
    ...
    \begin{subequations}
        ...
        \begin{align}
            C_1' &= -(k_1+k_2)C_1+k_3C_2+u \\
            C_2' &= k_2C_1-(k_3+k_4)C_2 \\
            y &= C_1
        \end{align}
    \end{subequations}
    \vfill
    ...
    \[
        G(s) = \frac{s+k_3+k_4}{s^2+(k_1+k_2+k_3+k_4)s+(k_1+k_2)(k_3+k_4)-k_2k_3}
    \]
\end{frame}
