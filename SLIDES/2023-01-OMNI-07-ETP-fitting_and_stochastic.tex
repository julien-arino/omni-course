...
\begin{frame}{An example adapted from Bellman and \r{A}str\"om (MBS 1970)}
    Inject a tracer in compartment $C_1$ and take samples from same compartment
    \vfill
    \begin{minipage}{0.3\textwidth}
        \begin{center}
            ...
            \node [circle, fill=gray!10, text=black] at (0\horzskip,1\vertskip) (C1) {$C_1$};
            \node [circle, fill=gray!10, text=black] at (0\horzskip,-1\vertskip) (C2) {$C_2$};
            %% Flows between
            \path [line, very thick, bend left] (C1) to node [midway, right] (TextNode) {$k_{12}C_1$} (C2);
            \path [line, very thick, bend left] (C2) to node [midway, left] (TextNode) {$k_{21}C_2$} (C1);
            %% Flows in
            \path [line, very thick] (-0.75\horzskip,1\vertskip) to node [midway, above] (TextNode) {$u$} (C1);
            %% Flows out of
            \path [line, very thick] (C1) to node [midway, above] (TextNode) {$k_{10}C_1$} (1\horzskip,1\vertskip);
            \path [line, very thick] (C2) to node [midway, above] (TextNode) {$k_{20}C_2$} (1\horzskip,-1\vertskip);
            \end{tikzpicture}    
        \end{center}
    \end{minipage}
    \begin{minipage}{0.64\textwidth}
        \begin{itemize}
            \item $C_1$ mass of drug in blood system
            \item $x_2$ mass of drug in gastro-intestinal system
            \item $y$ observed concentration of drug (blood test)
            \item $V_1$ volume of observed compartment (blood system)
            \item $k_{ij}$ rates of transfer from $i$ to $j$ 
            \item $u$ injection rate
        \end{itemize}
    \end{minipage}
\end{frame}
...
%           \node [circle, fill=gray!10, text=black] at (-1\horzskip,0\vertskip) (C1) {$C_1$};
%           \node [circle, fill=gray!10, text=black] at (1\horzskip,0\vertskip) (C2) {$C_2$};
%           %% Flows between
%           \path [line, very thick, bend left] (C1) to node [midway, above] (TextNode) {$k_{12}C_1$} (C2);
%           \path [line, very thick, bend left] (C2) to node [midway, below] (TextNode) {$k_{21}C_2$} (C1);
%           %% Flows in
%           \path [line, very thick] (-1.75\horzskip,0\vertskip) to node [midway, above] (TextNode) {$u$} (C1);
%           %% Flows out of
%           \path [line, very thick] (C1) to node [midway, left] (TextNode) {$k_{10}C_1$} (-1\horzskip,1\vertskip);
%           \path [line, very thick] (C2) to node [midway, right] (TextNode) {$k_{20}C_2$} (1\horzskip,1\vertskip);
%         \end{tikzpicture}    
%     \end{center} 
%     \begin{subequations}
%         \label{sys:BellmanAstrom}
%         \begin{align}
%             C_1' &= -(k_{10}+k_{12})C_1+k_{21}C_2+u \\
%             C_2' &= k_{12}C_1-(k_{21}+k_{20})C_2 \\
%             y &= C_1
%         \end{align}
%     \end{subequations}
%     \vfill
%     Take Laplace transform (could also take matrix exponential), giving transfer function
%     \[
%         G(s) = \frac{s+k_{21}+k_{20}}{s^2+(k_{10}+k_{12}+k_{21}+k_{20})s+(k_{10}+k_{12})(k_{21}+k_{20})-k_{12}k_{21}}
%     \]
% \end{frame}


\begin{frame}{The model for a single injection}
	\begin{minipage}{0.3\textwidth}
		\begin{center}
            \def\vertskip{*1.75}
            \def\horzskip{*2}
	...
	\begin{minipage}{0.68\textwidth}
        \begin{subequations}
            \label{sys:BellmanAstrom}
            \begin{align}
                C_1' &= -(k_{12}+k_{10})C_1+k_{21}C_2 \label{sys:BellmanAstrom_dC1} \\
                C_2'&= k_{12}C_1-(k_{21}+k_{12})C_2 \label{sys:BellmanAstrom_dC2} \\
                C_1(0)&=D, C_2(0)=0 \label{sys:BellmanAstrom_IC} \\
                y&=C_1/V_1  \label{sys:BellmanAstrom_obs} 
            \end{align}
        \end{subequations}
	\end{minipage}
	\vfill
    Rather than continuous input, assume here ``one shot'', $C_1(0)=D$
    \vfill
	5 parameters ($V_1$ and $k_{ij}$) to estimate
\end{frame}
